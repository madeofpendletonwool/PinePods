...
def call_backup_server(url, headers, backup_dir, database_pass):
    ...
    try:
        response = requests.get(url + "/backup_server", headers=headers, timeout=60)  # Increase timeout since it might take a while
        response.raise_for_status()
        ...
        with open(os.path.join(backup_dir, "pinepods_server_backup.sql"), 'wb') as file:
