    ...
    match &state.db_pool {
        crate::database::DatabasePool::Postgres(pool) => {
            ...
            for result in rows {
                ...
                let podcast_name = result
                    .try_get::<String, _>("podcastname")
                    .unwrap_or_else(|_| format!("Podcast {}", podcast_id));
                println!(
                    "Running refresh for podcast {}/{}: {}",
                    current_podcast, total_podcasts, podcast_name
                );
                ...
                if is_youtube {
                        ...
                        .await
                    {
                        Ok(new_episodes) => {
                            println!(
                                "Successfully refreshed podcast {}: {} new episodes",
                                podcast_id,
                                new_episodes.len()
                            );
                            ...
                            if auto_download {
                                ...
                                for episode in &new_episodes {
                                    println!(
                                        "Auto-downloading episode '{}' (ID: {}) for user {}",
                                        episode.episodetitle, episode.episodeid, user_id
                                    );

                                    // Determine if this is a YouTube episode
                                    let is_youtube = episode.episodeurl.contains("youtube.com")
                                        || episode.episodeurl.contains("youtu.be");

                                    // Spawn download task
                                    let task_result = if is_youtube {
                                        state
                                            .task_spawner
                                            .spawn_download_youtube_video(
                                                episode.episodeid,
                                                user_id,
                                            )
                                            .await
                                        ...
                                        state
                                            .task_spawner
                                            .spawn_download_podcast_episode(
                                                episode.episodeid,
                                                user_id,
                                            )
                                            .await
                                    ...
                                    match task_result {
                                        Ok(task_id) => println!(
                                            "Auto-download task queued with ID: {}",
                                            task_id
                                        ),
                                        Err(e) => println!(
                                            "Failed to queue auto-download task for episode {}: {}",
                                            episode.episodeid, e
                                        ),
                                    }
                                }
...
async fn run_admin_gpodder_sync(
    state: &AppState,
    user_id: i32,
    sync_type: &str,
) -> AppResult<SyncResult> {
    match sync_type {
        "nextcloud" => match state.db_pool.sync_with_nextcloud_for_user(user_id).await {
            Ok(success) => {
                if success {
                    Ok(SyncResult {
                        synced_podcasts: 1,
        ...
        "gpodder" | "external" | "both" => match state.db_pool.gpodder_sync(user_id).await {
            Ok(sync_result) => Ok(SyncResult {
...
pub async fn websocket_refresh_episodes(
    ...
    if !check_user_access(&state, &api_key, user_id).await? {
        return Err(AppError::forbidden(
            "You can only refresh your own podcasts",
        ));
    }
...
async fn handle_refresh_websocket(
    ...
    // Check if refresh is already running for this user
    {
        ...
        if locks.contains_key(&user_id) {
            let _ =
                send_error_and_close(socket, "Refresh job already running for this user.").await;
            return;
        }
    }
...
async fn send_error_and_close(mut socket: WebSocket, error: &str) -> Result<(), AppError> {
    let error_msg = RefreshMessage::Error {
...
async fn run_refresh_process(
    ...
    println!(
        "Starting refresh process for user_id: {}, nextcloud_refresh: {}",
        user_id, nextcloud_refresh
    );
    ...
    if nextcloud_refresh {
        ...
        if gpodder_status.sync_type != "None" && !gpodder_status.sync_type.is_empty() {
            println!(
                "gPodder sync is enabled for user {}, sync_type: {}",
                user_id, gpodder_status.sync_type
            );
            ...
            let _ = tx
                .send(RefreshMessage::Status(RefreshStatus {
                    progress: RefreshProgress {
                        current: 0,
                        total: 1,
                        current_podcast: format!(
                            "Syncing with gPodder ({})...",
                            gpodder_status.sync_type
                        ),
                    },
                }))
                .await;
            ...
            match handle_gpodder_sync(&state, user_id, &gpodder_status.sync_type).await {
                Ok(sync_result) => {
                    println!(
                        "gPodder sync successful for user {}: {} podcasts, {} episodes",
                    ...
                    let _ = tx
                        .send(RefreshMessage::Status(RefreshStatus {
                            progress: RefreshProgress {
                                ...
                                total: 1,
                                current_podcast: format!(
                                    "gPodder sync completed: {} podcasts, {} episodes",
            ...
            println!(
                "gPodder sync not enabled for user {} (enabled: {}, type: {})",
                user_id,
                gpodder_status.sync_type != "None" && !gpodder_status.sync_type.is_empty(),
                gpodder_status.sync_type
            );
        }
    }
    ...
    println!(
        "Found {} podcasts to refresh for user {}",
        total_podcasts, user_id
    );
    ...
    for podcast in podcasts {
        ...
        println!(
            "Refreshing podcast {}/{}: {} (ID: {}, is_youtube: {})",
    ...
    let _ = tx
        .send(RefreshMessage::Status(RefreshStatus {
            progress: RefreshProgress {
                ...
                total: total_podcasts,
                current_podcast: format!(
                    "Refresh completed: {}/{} successful, {} new episodes",
...
async fn refresh_rss_feed(
    ...
    if podcast.auto_download {
        tracing::info!(
            "Auto-download enabled for podcast '{}' - processing {} new episodes",
            podcast.name,
            new_episodes.len()
        );
        ...
        for episode in &new_episodes {
            ...
            let task_result = if is_youtube {
                state
                    .task_spawner
                    .spawn_download_youtube_video(episode.episodeid, user_id)
                    .await
            } else {
                state
                    .task_spawner
                    .spawn_download_podcast_episode(episode.episodeid, user_id)
                    .await
            };
    ...
    if user_id != 0 {
        tracing::info!(
            "Refreshed podcast '{}' for user {}: {} new episodes",
            podcast.name,
            user_id,
            new_episodes.len()
        );
...
async fn handle_gpodder_sync(
    state: &AppState,
    user_id: i32,
    sync_type: &str,
) -> AppResult<SyncResult> {
    println!(
        "Starting gPodder sync for user {}, sync_type: {}",
        user_id, sync_type
    );
    ...
    match sync_type {
        ...
        "gpodder" | "external" | "both" => {
            println!(
                "Performing standard gPodder sync for user {}, type: {}",
                user_id, sync_type
            );
            ...
            match state.db_pool.gpodder_sync(user_id).await {
                Ok(sync_result) => {
                    println!(
                        "Standard gPodder sync successful for user {}: {} podcasts, {} episodes",
                        user_id, sync_result.synced_podcasts, sync_result.synced_episodes
                    );
        ...
        _ => {
            println!(
                "Unknown sync type '{}' for user {}, skipping sync",
                sync_type, user_id
            );
            Ok(SyncResult {
                synced_podcasts: 0,
...
pub async fn refresh_gpodder_subscriptions_admin_internal(state: &AppState) -> AppResult<()> {
    ...
    let gpodder_users = state.db_pool.get_all_users_with_gpodder_sync().await?;
    tracing::info!(
        "Found {} users with GPodder sync enabled",
        gpodder_users.len()
    );
    ...
    for user_id in gpodder_users.iter() {
        ...
        if gpodder_status.sync_type != "None"
            && gpodder_status.sync_type != "nextcloud"
            && !gpodder_status.sync_type.is_empty()
        {
            match run_admin_gpodder_sync(state, *user_id, &gpodder_status.sync_type).await {
                Ok(_) => {
                    successful_syncs += 1;
    ...
    tracing::info!(
        "Internal GPodder sync completed: {}/{} users successful",
        successful_syncs,
        gpodder_users.len()
    );
        ...
        .spawn_progress_task(
            ...
            move |reporter| async move {
                ...
                reporter
                    .update_progress(
                        10.0,
                        Some("Starting Nextcloud sync for all users...".to_string()),
                    )
                    .await?;

                // Get all users who have Nextcloud sync enabled
                let nextcloud_users = state
                    .db_pool
                    .get_all_users_with_nextcloud_sync()
                    .await
                    .map_err(|e| {
                        AppError::internal(&format!("Failed to get Nextcloud users: {}", e))
                    })?;

                println!(
                    "Found {} users with Nextcloud sync enabled",
                    nextcloud_users.len()
                );
                ...
                if total_users == 0 {
                    reporter
                        .update_progress(
                            100.0,
                            Some("No users with Nextcloud sync found".to_string()),
                        )
                        .await?;
                ...
                for (index, user_id) in nextcloud_users.iter().enumerate() {
                    ...
                    reporter
                        .update_progress(
                            progress,
                            Some(format!("Running Nextcloud sync for user {}", user_id)),
                        )
                        .await?;
                ...
                reporter
                    .update_progress(
                        100.0,
                        Some(format!(
                            "Nextcloud sync completed: {}/{} users successful",
                            successful_syncs, total_users
                        )),
                    )
                    .await?;
...
pub async fn refresh_nextcloud_subscriptions_admin_internal(state: &AppState) -> AppResult<()> {
    ...
    tracing::info!(
        "Internal Nextcloud sync completed: {}/{} users successful",
        successful_syncs,
        nextcloud_users.len()
    );
