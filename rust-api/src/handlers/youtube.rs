...
pub async fn search_youtube_channels(
    ...
    if key_id != query.user_id && !is_web_key {
        return Err(AppError::forbidden(
            "You can only search with your own account.",
        ));
    }
            ...
            if let Some(videos) = channel_videos.get_mut(channel_id) {
                if videos.len() < 3 {
                    ...
                    if let Some(video_id) = entry.get("id").and_then(|v| v.as_str()) {
                        ...
                        videos.push(video);
                        println!(
                            "Added video to channel {}, now has {} videos",
                            channel_id,
                            videos.len()
                        );
                    }
...
pub async fn subscribe_to_youtube_channel(
    ...
    let existing_id = state
        .db_pool
        .check_existing_channel_subscription(&query.channel_id, query.user_id)
        .await?;
    ...
    let podcast_id = state
        .db_pool
        .add_youtube_channel(&channel_info, query.user_id, feed_cutoff)
        .await?;
    ...
    tokio::spawn(async move {
        if let Err(e) =
            process_youtube_channel(podcast_id, &channel_id_clone, feed_cutoff, &state_clone).await
        {
            println!(
                "Error processing YouTube channel {}: {}",
                channel_id_clone, e
            );
        }
    });
...
async fn get_youtube_channel_info(channel_id: &str) -> Result<HashMap<String, String>, AppError> {
    println!(
        "Getting channel info for {} from Backend service",
        channel_id
    );
    ...
    let backend_url = search_api_url.replace(
        "/api/search",
        &format!("/api/youtube/channel?id={}", channel_id),
    );

    let client = reqwest::Client::new();
    let response =
        client.get(&backend_url).send().await.map_err(|e| {
            AppError::external_error(&format!("Failed to call Backend service: {}", e))
        })?;
    ...
    let channel_data: serde_json::Value = response.json().await.map_err(|e| {
        AppError::external_error(&format!("Failed to parse Backend response: {}", e))
    })?;
    ...
    println!(
        "Successfully extracted channel info for: {}",
        channel_info.get("name").unwrap_or(&"Unknown".to_string())
    );
    Ok(channel_info)
}
...
pub async fn process_youtube_channel(
    ...
    state
        .db_pool
        .remove_old_youtube_videos(podcast_id, cutoff_date)
        .await?;
    ...
    println!(
        "Fetching channel data from Backend service: {}",
        backend_url
    );
    ...
    let empty_vec = vec![];
    let recent_videos_data = channel_data
        .get("recentVideos")
        .and_then(|v| v.as_array())
        .unwrap_or(&empty_vec);

    println!(
        "Found {} total videos from Backend service",
        recent_videos_data.len()
    );
    ...
    for video_entry in recent_videos_data {
        ...
        if published <= cutoff_date {
            println!(
                "Video {} from {} is too old, stopping processing",
                video_id, published
            );
            break;
        }
    ...
    println!(
        "Processing complete - Found {} recent videos",
        recent_videos.len()
    );
    ...
    if !recent_videos.is_empty() {
        ...
        // Get existing videos
        let existing_videos = state
            .db_pool
            .get_existing_youtube_videos(podcast_id)
            .await?;
        ...
        if !new_videos.is_empty() {
            state
                .db_pool
                .add_youtube_videos(podcast_id, &new_videos)
                .await?;
            ...
            match download_youtube_audio(video_id, &output_path).await {
                Ok(_) => {
                    ...
                    if let Some(duration) = get_mp3_duration(&output_path) {
                            ...
                            .await
                        {
                            ...
                            println!(
                                "Updated duration for video {} to {} seconds",
                                video_id, duration
                            );
                        }
                    } else {
                ...
                Err(e) => {
                    ...
                    if error_msg.to_lowercase().contains("members-only") {
                        println!(
                            "Skipping video {} - Members-only content: {}",
                            video_id, title
                        );
...
pub async fn check_youtube_channel(
    ...
    if key_id != query.user_id && !is_web_key {
        return Err(AppError::forbidden(
            "You can only check channels for yourself!",
        ));
    }

    let exists = state
        .db_pool
        .check_youtube_channel(query.user_id, &query.channel_name, &query.channel_url)
        .await?;
