...
pub async fn gpodder_set_default(
    ...
    let success = state
        .db_pool
        .gpodder_set_default_device(user_id, device_id)
        .await?;
...
pub async fn gpodder_force_sync(
    ...
    let sync_result = match settings.sync_type.as_str() {
        "gpodder" => {
            ...
            state
                .db_pool
                .call_gpodder_initial_full_sync(
                    user_id,
                    "http://localhost:8042",
                    &settings.username,
                    &settings.token,
        ...
        "nextcloud" => {
            ...
            state
                .db_pool
                .call_nextcloud_initial_full_sync(
                    user_id,
                    &settings.url,
                    &settings.username,
                    &settings.token,
        ...
        "external" => {
            ...
            let decrypted_token = state
                .db_pool
                .decrypt_password(&settings.token)
                .await
                .unwrap_or_default();
            state
                .db_pool
        ...
        "both" => {
            ...
            match (internal_result, external_result) {
                (Ok(internal_success), Ok(external_success)) => {
                    Ok(internal_success || external_success)
                }
                (Ok(internal_success), Err(external_err)) => {
                    tracing::warn!(
                        "External sync failed: {}, but internal sync succeeded: {}",
                        external_err,
                        internal_success
                    );
                    Ok(internal_success)
                ...
                (Err(internal_err), Ok(external_success)) => {
                    tracing::warn!(
                        "Internal sync failed: {}, but external sync succeeded: {}",
                        internal_err,
                        external_success
                    );
                    Ok(external_success)
                ...
                (Err(internal_err), Err(external_err)) => {
                    tracing::error!(
                        "Both internal and external sync failed: internal={}, external={}",
                        internal_err,
                        external_err
                    );
                    Err(internal_err)
    ...
    if success {
        ...
        let message = error_message.unwrap_or_else(|| {
            "Initial sync failed - please check your sync configuration".to_string()
        });
        Ok(Json(serde_json::json!({
            "success": false,
...
pub async fn gpodder_sync(
    ...
    let sync_result = state
        .db_pool
        .refresh_gpodder_subscription_background(user_id)
        .await?;
...
pub async fn gpodder_test_connection(
    ...
    if user_id != user_id_from_api_key && !is_web_key {
        return Err(AppError::forbidden(
            "You can only test connections for yourself!",
        ));
    }
...
pub async fn gpodder_create_device(
    ...
    let settings = state
        .db_pool
        .get_user_sync_settings(user_id)
        .await?
        .ok_or_else(|| {
            AppError::BadRequest("User not found or GPodder sync not configured".to_string())
        })?;

    // Validate that GPodder sync is enabled
    if settings.sync_type != "gpodder" && settings.sync_type != "both" {
        return Err(AppError::BadRequest(
            "GPodder sync is not enabled for this user".to_string(),
        ));
    }
    ...
    let device_id = state
        ...
        .await
        .map_err(|e| {
            AppError::Internal(format!("Failed to create device via GPodder API: {}", e))
        })?;
