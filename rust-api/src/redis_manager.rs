...
impl ImportProgressManager {
    ...
    pub async fn start_import(&self, user_id: i32, total_podcasts: i32) -> AppResult<()> {
        ...
        self.redis_client
            .set_ex(&key, &progress_data.to_string(), 3600)
            .await?;
    ...
    pub async fn update_progress(
        ...
        if let Some(progress_json) = self.redis_client.get::<String>(&key).await? {
            if let Ok(mut progress) = serde_json::from_str::<Value>(&progress_json) {
                ...
                progress["current_podcast"] =
                    serde_json::Value::String(current_podcast.to_string());

                self.redis_client
                    .set_ex(&key, &progress.to_string(), 3600)
                    .await?;
            }
        }
...
impl NotificationManager {
    ...
    pub async fn send_test_notification(
        ...
        println!(
            "Sending test notification for user {} on platform {}",
            user_id, platform
        );
    ...
    async fn send_ntfy_notification(&self, settings: &serde_json::Value) -> AppResult<bool> {
        ...
        if !is_success {
            ...
            println!(
                "Ntfy notification failed with status: {} - Response: {}",
    ...
    async fn send_gotify_notification(&self, settings: &serde_json::Value) -> AppResult<bool> {
        let gotify_url = settings
            .get("gotify_url")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        let gotify_token = settings
            .get("gotify_token")
            .and_then(|v| v.as_str())
            .unwrap_or("");
