...
def refresh_pods(cnx, database_type):
    ...
    for result in result_set:
        # Check if result is a tuple (default for psycopg)
        if isinstance(result, tuple):
            podcast_id, feed_url, artwork_url, auto_download = result
        # Check if result is a dict (possible if using DictCursor)
        elif isinstance(result, dict):
            podcast_id = result.get("PodcastID")
            feed_url = result.get("FeedURL")
            artwork_url = result.get("ArtworkURL")
            auto_download = result.get("AutoDownload")
        else:
