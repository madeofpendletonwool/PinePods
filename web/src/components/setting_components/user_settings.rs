...
use crate::requests::setting_reqs::{SettingsUser, call_add_user, AddSettingsUserRequest};
...

    {
        ...
        use_effect(move || {
            let future = async move {
                let response = call_get_user_info(server_name.unwrap(), api_key.unwrap()).await;
                match response {
                    Ok(user_info) => {
                        users.set(user_info);
                    },
